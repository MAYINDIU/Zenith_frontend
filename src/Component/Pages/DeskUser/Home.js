            ...
            <Link key={i} to="#">
              <div className="shadow-md bordered text-white rounded p-2 lg:p-4 rounded bordered bg-[#0E9F6E] max-w-sm">
                <h5 className="font-normal mt-1">{mName?.module_name}</h5>
                 <div className='mt-5'>
                 <span className='ml-1 text-white bg-[#00838F] rounded p-2'> {mName?.p_read===1?"VIEW":null} </span>
                <span className='ml-1 text-white bg-[#00838F] rounded p-2'> {mName?.p_create===2?"CREATE":null} </span>
                <span className='ml-1 text-white bg-[#00838F] rounded p-2'>{mName?.p_edit===3?"EDIT":null} </span>
                <span className='ml-1 text-white bg-[#00838F] rounded p-2'>{mName?.p_delete===4?"DELETE":null}</span>
