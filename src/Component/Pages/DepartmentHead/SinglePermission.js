...
console.log(selectedPermissions)
  ...
  const PrevList = async () => {
    setSpinner(true);
    try {
      const response = await axios.get(`http://localhost:5000/api/single-privilage-list/${id}/${name}`);
  ...
  return (
    <div>
      <Navbar />
      <h1 className='shadow w-1/5 mx-auto p-3 mt-5 font-bold rounded text-center'>UPDATE USER PERMISSIONS</h1>
      ...
      {permission?.map((p, i) => (
        <div key={i} className=' p-3 w-full lg:w-1/4 mt-5 mx-auto  text-center'>
          <div class="max-w-xl p-6 bg-white border border-gray-200 rounded-md shadow dark:bg-gray-800 dark:border-gray-700">
            <h1 className='font-bold text-lg'>{p?.name}  <span className='text-sm'>( {p?.dep_name})</span> </h1>
            <h1 className='font-bold mt-5 text-center'>Module Name:  {p?.module_name}</h1>
            ...
            <button type="button" class="px-3 py-2 text-xs font-medium text-center text-white bg-green-700 rounded hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-blue-800">UPDATE</button>
            <button color="failure" type="button" class="px-3 ml-1 py-2 text-xs font-medium text-center text-white bg-[#DD2C00] rounded hover:bg-[#BF360C] focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-blue-800">DELETE</button>
